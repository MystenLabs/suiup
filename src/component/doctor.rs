...
use crate::paths::{get_suiup_data_dir, get_default_bin_dir, installed_binaries_file, default_file_path};
...
pub async fn run_doctor_checks() -> Result<()> {
    ...
    let mut check = |message: &str, result: Result<String, String>| {
        match result {
            ...
            Err(e) => {
                if e.starts_with("WARN:") {
                    warnings += 1;
                    println!("[{}] {}", "!".yellow(), e.strip_prefix("WARN:").unwrap_or(&e).trim());
                } else {
                    errors += 1;
                    println!("[{}] {}", "✗".red(), e.strip_prefix("ERROR:").unwrap_or(&e).trim());
                }
            }
        }
    };
    ...
    if errors > 0 {
        println!("{}", format!("Found {} error(s) and {} warning(s).", errors, warnings).red());
    } else if warnings > 0 {
...
fn check_suiup_data_dir() -> Result<String, String> {
    ...
    if path.exists() && path.is_dir() {
        ...
        Err(format!("ERROR: suiup data directory not found at {}", path.display()))
    }
}

fn check_path_variables(check: &mut impl FnMut(&str, Result<String, String>)) {
    let default_bin_dir = get_default_bin_dir();
    check("Default binary directory", Ok(format!("is {}", default_bin_dir.display())));

    match env::var("PATH") {
        Ok(path_var) => {
            let paths: Vec<_> = env::split_paths(&path_var).collect();
            if !paths.contains(&default_bin_dir) {
                check("Default binary directory in PATH", Err("WARN: Not found in PATH. Binaries managed by suiup may not be accessible.".to_string()));
            } else {
        ...
        Err(_) => {
            check("PATH variable", Err("ERROR: Could not read PATH environment variable.".to_string()));
        }
    }
}
...
fn check_config_files(check: &mut impl FnMut(&str, Result<String, String>)) {
    ...
    match installed_path {
        Ok(path) => {
            if !path.exists() {
                check("Installed binaries config", Err(format!("WARN: File not found at {}", path.display())));
            } else {
                match InstalledBinaries::read_from_file() {
                    Ok(_) => check("Installed binaries config", Ok("is valid".to_string())),
                    Err(e) => check("Installed binaries config", Err(format!("ERROR: Failed to parse: {}", e))),
                }
            }
        }
        Err(e) => check("Installed binaries config", Err(format!("ERROR: Could not get path: {}", e))),
    }
    ...
    let default_path = default_file_path();
     match default_path {
        Ok(path) => {
            if !path.exists() {
                check("Default version config", Err(format!("WARN: File not found at {}", path.display())));
            } else {
                 match std::fs::read_to_string(&path) {
                    Ok(content) => {
                        ...
                        if result.is_ok() {
                            ...
                            check("Default version config", Err("ERROR: Failed to parse as valid JSON.".to_string()));
                        }
                    },
                    Err(e) => check("Default version config", Err(format!("ERROR: Failed to read: {}", e))),
                }
            }
        }
        Err(e) => check("Default version config", Err(format!("ERROR: Could not get path: {}", e))),
    }
}
...
fn check_dependencies(check: &mut impl FnMut(&str, Result<String, String>)) {
    ...
    match Command::new("rustc").arg("--version").output() {
        ...
        _ => check("rustc", Err("WARN: Not found. Required for --nightly builds.".to_string())),
    }
    ...
    match Command::new("cargo").arg("--version").output() {
        ...
        _ => check("cargo", Err("WARN: Not found. Required for --nightly builds.".to_string())),
    }
    ...
    match Command::new("git").arg("--version").output() {
        ...
        _ => check("git", Err("WARN: Not found. Required for --nightly builds.".to_string())),
    }
}
...
async fn check_network_connectivity(check: &mut impl FnMut(&str, Result<String, String>)) {
    match reqwest::get("https://api.github.com").await {
        Ok(resp) if resp.status().is_success() => check("GitHub API connectivity", Ok("".to_string())),
        _ => check("GitHub API connectivity", Err("ERROR: Cannot connect to GitHub API. Downloads will fail.".to_string())),
    }
}