...
impl ConfigValue {
    pub fn from_string(key: &str, value: &str) -> Result<Self> {
        match key {
            "default_network" | "install_path" | "github_token" => {
...
impl ConfigHandler {
    ...
    fn load_config() -> Result<SuiupConfig> {
        ...
        if !config_path.exists() {
            ...
            Self::save_config(&default_config)
                .with_context(|| format!("Failed to create default configuration file"))?;
        ...
        let config: SuiupConfig = serde_json::from_str(&content)
            .with_context(|| format!("Configuration file contains invalid JSON. Try 'suiup config reset' to restore defaults"))?;
    ...
    fn save_config(config: &SuiupConfig) -> Result<()> {
        ...
        if let Some(parent) = config_path.parent() {
            fs::create_dir_all(parent)
                .with_context(|| format!("Failed to create configuration directory at {}", parent.display()))?;
        }

        let content = serde_json::to_string_pretty(config)
            .with_context(|| format!("Failed to serialize configuration"))?;

        fs::write(&config_path, content).with_context(|| format!("Failed to write configuration file"))?;
    ...
    pub async fn set(&mut self, key: &str, value: ConfigValue) -> Result<()> {
        ...
        match key {
            ...
            "github_token" => {
                if let ConfigValue::String(ref v) = value {
                    self.config.github_token = if v.is_empty() { None } else { Some(v.clone()) };
                }
            }
            _ => bail!("Unknown configuration key: {}", key),
        }
    ...
    pub async fn unset(&mut self, key: &str) -> Result<()> {
        ...
        Self::save_config(&self.config)?;
        println!( "Configuration key '{}' reset to default",key.cyan());
        Ok(())
    }
